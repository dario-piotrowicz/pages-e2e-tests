...
const combinableDeploymentConfigObjects = {
		...
		.record(
			z
				.object({
					production: z
						.object({
							id: hex32.describe("The ID of the Durable Object namespace."),
						})
		...
		.record(
			z
				.object({
					production: z
						.object({
							name: z.string().nonempty().describe("The name of the Service."),
							environment: z
								.string()
								.nonempty()
		...
		.record(
			z
				.object({
					production: z
						.object({
							name: z.string().nonempty().describe("The name of the Queue."),
						})
...
export const featuresSchema = z.object({
	...
	deploymentConfig: z
		...
		.default({})
		.describe(
			"The additional deployment configuration to use when deploying this feature."
		),
});

export type FeatureConfig = z.TypeOf<typeof featuresSchema>;
